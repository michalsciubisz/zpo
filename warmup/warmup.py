...
def f_default_arg_mutable(x: int, data: [int] = None):
    if data == None:
        data = []
...
def f_str_formatting(array: [float]):
     string = ""
     for item in array:
         string_helping = str("%.2f" % round(item, 2))
         string += string_helping
         if array[-1] != item:
             string += " - "
     return string
...
def f_comprehensions(dct: Dict[int, int]) -> Tuple[int, Set[int]]:
    ...
    return (suma, set(elem for elem in range(1,6) if elem in dct))
