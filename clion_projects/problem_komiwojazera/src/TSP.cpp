 ...
 */
// TODO: Implement it!
path_t StageState::get_path() { //dziala
    path_t sorted_path;
    ...
    while(sorted_path.size() < matrix_.size()){
        for(std::size_t i = 0; i < unsorted_path_.size(); i++ ){
            if(sorted_path.back() == unsorted_path_[i].row){
...
cost_t CostMatrix::get_vertex_cost(std::size_t row, std::size_t col) const { //dziala

    auto min_val_row = INF;
    auto min_val_col = INF;

    for (std::size_t i = 0; i <matrix_.size() ; i++) {

        for (std::size_t j = 0; j <matrix_.size() ; j++) {
            if (i == row && j != col && matrix_[row][j] <= min_val_col && matrix_[row][j] != INF)
                min_val_col = matrix_[i][j];

            if (i != row && j == col && matrix_[i][col] <= min_val_row && matrix_[i][col] != INF)
...
NewVertex StageState::choose_new_vertex() {
    NewVertex reduced_vertex = NewVertex(vertex_t(0, 0), -1);
    for (std::size_t i = 0; i < matrix_.size(); i++) {
        for (std::size_t j = 0; j < matrix_.size(); j++) {
            if(matrix_[i][j] == 0){
                vertex_t vertex = vertex_t(i, j);
                cost_t cost = matrix_.get_vertex_cost(vertex.row, vertex.col);
                if(reduced_vertex.cost < cost){
                    reduced_vertex.coordinates = vertex;
                    reduced_vertex.cost = cost;
                }
            }
        }
    }
    return reduced_vertex;
}
...
void StageState::update_cost_matrix(vertex_t new_vertex){
    auto main = new_vertex.row;
    bool finded = true;
    auto under_main = new_vertex.col;
    bool get_sure_finded = true;
    ...
    while (finded) {
        ...
        {
            if (wierzcholek.col == main)
            {
                finded = true;
                main = wierzcholek.row;
                break;
            }
            else
                finded = false;
    ...
    while (get_sure_finded) {
        for (auto apex : unsorted_path_)
        {
            if (apex.row == under_main)
            {
                under_main = apex.col;
                get_sure_finded = true;
                break;
            }
            else
                get_sure_finded = false;
        }
    }
    matrix_[under_main][main] = INF;
}
